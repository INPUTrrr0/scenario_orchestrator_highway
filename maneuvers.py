...
def retime_actor(a: se.Actor, v_target: float) -> None:
    ...
    for m in a.maneuvers:
        ...
        elif m.type == "lane_change":
            dist = _geom_distance(m)
            if dist <= 1e-6:
                continue
            out.append(se.Maneuver(type="lane_change", intercept=v_target,
...
def apply_perturbation(sc: se.Scenario, p: Perturbation) -> se.Scenario:
    ...
    new = se.Scenario(map=se.clone_map(sc.map), actors=actors,
