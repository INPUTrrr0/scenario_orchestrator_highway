...
import directives as dv               # noqa: E402  (rect geometry for collision scan)
...
def _rollout_end(sc: se.Scenario, ego: str, cap: float) -> float:
    """Clip length for a node: run to the ego's collision (+0.8s, min 2.5s so a
    just-collided node still shows something), else the full rollout period."""
    DT = se.DT
    ego_a = next((a for a in sc.actors if a.id == ego), None)
    if ego_a is not None and ego_a.traj:
        n = min(len(ego_a.traj), int(cap / DT) + 1)

        def body(a, k):
            k = min(k, len(a.traj) - 1)
            x, y, h = a.traj[k]
        ...
        for k in range(n):
            ...
            for a in sc.actors:
                if a.id == ego:
                    continue
                if dv.rects_overlap(eb, body(a, k)):
                    return min(cap, max(2.5, k * DT + 0.8))
    return min(cap, max(0.5, sc.period))
...
def render_snapshot(sc_path: str, out_mp4: str, signals: dict, ego: str,
    ...
    # play the full realization: to the ego collision (+ a beat), else the rollout
    t_end = _rollout_end(sc, ego, cap=horizon)
    n = int(t_end * fps) + 1
    hdr = 40
...
(function legend(){
  ...
  const order=["start","perturbation","intervention","checkpoint","proposal","collision","infeasible"];
...
def main():
    ...
    ap.add_argument("--horizon", type=float, default=12.0,
                    help="max clip length (s); clips run to the collision + a beat")
