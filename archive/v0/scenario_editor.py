...
Scenario Editor
===============
Load an orchestrated driving scenario (YAML), visualize it in a bird's-eye
...
Model (see DESIGN.md):
  ...
  * Linear timing curve whose meaning is per-maneuver:
        ...
        - accelerate/decelerate/lane_change -> velocity vs time
  ...
  * Map: either a 4-way intersection (default) or a straight multi-lane road
    (map.kind: straight) for highway-style scenes such as cut-ins.
...
Coordinates: graph/Cartesian, meters, origin at map center,
...
# offset (positive = left / CCW of heading, negative = right). Heading returns
# to the start heading at the end so the next segment chains cleanly.
...
MANEUVER_TYPE_CYCLE = ["go_straight", "lane_change", "turn_left", "turn_right",
...
class Maneuver:
    ...
    def distance(self, t: float) -> float:
        ...
        # longitudinal / lane_change: integrate v(t)=intercept+slope*t, v >= 0
        t_eff = t
        if self.slope < 0:
    ...
    def pose_at(self, start: Pose, t: float) -> Pose:
        ...
        if self.type == "lane_change":
            ...
            # smoothstep: 0 at start, 1 at end, zero derivative at both ends
            s_lat = frac * frac * (3.0 - 2.0 * frac)
            along = self.distance(t)
            lat = self.lateral_offset * s_lat
            # left-normal of heading; brief yaw bump that returns to 0 at ends
...
@dataclass
class MapConfig:
    # kind: "intersection" (default 4-way cross) or "straight" (multi-lane strip)
...
class Scenario:
    ...
    def to_dict(self) -> dict:
        m: dict = {"kind": self.map.kind, "lane_width": self.map.lane_width}
        if self.map.kind == "straight":
            m["num_lanes"] = self.map.num_lanes
            m["length"] = self.map.length
        else:
            m["arm_length"] = self.map.arm_length
        return {"map": m,
    ...
    for ad in raw.get("actors", []):
        ...
        for md in ad.get("maneuvers", []):
            ...
            mans.append(Maneuver(
                ...
                lateral_offset=float(md.get("lateral_offset", mapcfg.lane_width)),
            ))
        actors.append(Actor(
            id=str(ad["id"]),
...
def run_gui(scenario: Scenario, persistence: Persistence,
    ...
    WIDTH = 1100
    TOPBAR_H = 56
    CANVAS_H = 620          # bird's-eye view region
    ...
    ADD_PRESETS_INTERSECTION = [
        (1.75, -58, 90), (58, 1.75, 180), (-58, -1.75, 0), (-1.75, 58, 270)]
    ADD_PALETTE = [(90, 190, 110), (210, 70, 60), (60, 120, 210),
                   (200, 160, 60), (160, 90, 200), (80, 200, 200)]

    def add_presets() -> List[Tuple[float, float, float]]:
        if scenario.map.kind == "straight":
            half_l = scenario.map.length / 2.0
            n = max(1, scenario.map.num_lanes)
            return [(scenario.map.lane_center_x(i % n), -half_l + 2.0, 90.0)
                    for i in range(n)]
        return ADD_PRESETS_INTERSECTION
    ...
    def do_add_actor() -> None:
        ...
        presets = add_presets()
        sx, sy, hd = presets[i % len(presets)]
    ...
    def do_cycle_maneuver_type() -> None:
        ...
        # fill sensible geometry defaults for turns / lane changes
    ...
    def draw_straight_map():
        lw = scenario.map.lane_width
        n = max(1, scenario.map.num_lanes)
        half_w = n * lw / 2.0
        half_l = scenario.map.length / 2.0
        x0, y0 = w2s(-half_w, half_l)
        x1, y1 = w2s(half_w, -half_l)
        pygame.draw.rect(screen, C_ROAD, pygame.Rect(x0, y0, x1 - x0, y1 - y0))
        # solid road edges
        pygame.draw.line(screen, C_EDGE, w2s(-half_w, -half_l), w2s(-half_w, half_l), 2)
        pygame.draw.line(screen, C_EDGE, w2s(half_w, -half_l), w2s(half_w, half_l), 2)
        # dashed lane dividers between same-direction lanes
        dash = 3.0
        for i in range(1, n):
            x = -half_w + i * lw
            d = -half_l
            while d < half_l:
                a1 = w2s(x, d)
                a2 = w2s(x, min(d + dash, half_l))
                pygame.draw.line(screen, C_LINE, a1, a2, 2)
                d += dash * 2

    def draw_intersection_map():
    ...
    def draw_subwindow():
        ...
        if m.curve_kind == "velocity":
            ...
            # right endpoint only steerable for accel/decel (flat types stay flat)
            ...
            if m.type == "lane_change":
                note = "lane change — lat_off +left / −right (m); speed & duration"
                screen.blit(font_sm.render(note, True, (150, 154, 165)),
                            (pr.x, pr.bottom + 22))
        else:
