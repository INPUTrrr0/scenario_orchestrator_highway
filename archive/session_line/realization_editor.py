...
prev/next, a `type:` dropdown, editable fields, and draggable velocity-curve
endpoints) — but drawn into a sub-region at a caller-chosen offset and driven by an
external event loop, instead of owning the whole window and loop. It edits a
`scenario_editor.Scenario` in place (the class the orchestrator kernel uses), calling
...
MANEUVER_TYPES = ["go_straight", "lane_change", "turn_left", "turn_right",
...
class RealizationEditor:
    ...
    def set_scenario(self, scenario: se.Scenario) -> None:
        ...
        self.dirty = False
        self.edits: List[str] = []
        self.dropdown: Optional[dict] = None
        self.mouse_pos = (0, 0)
    ...
    def do_set_maneuver_type(self, new_type: str) -> None:
        if self.selected is None:
            return
        a = self.scenario.actors[self.selected]
        ...
        if m.type in ("turn_left", "turn_right"):
            ...
            if m.angle <= 0:
                m.angle = 90.0
        if m.type == "lane_change":
            if abs(getattr(m, "lateral_offset", 0.0)) < 1e-6:
                m.lateral_offset = self.scenario.map.lane_width
    ...
    def open_dropdown(self, anchor: pygame.Rect, items, cb) -> None:
        if not items:
            return
        w = min(280, max(160, max(self.font_sm.size(lbl)[0] for lbl, _ in items) + 26))
        vis = min(len(items), DD_MAX_VIS)
        h = vis * DD_ITEM_H + 4
        x = min(anchor.x, self.W - w - 4)
        y = anchor.bottom + 2
        if y + h > self.H - 4:
            y = max(4, anchor.y - h - 2)
        self.dropdown = {"rect": pygame.Rect(x, y, w, h), "items": items,
                         "cb": cb, "scroll": 0}

    def dropdown_click(self, mx: int, my: int) -> None:
        dd = self.dropdown
        self.dropdown = None
    ...
    def _draw_dropdown(self) -> None:
        if self.dropdown is None:
            return
        s = self._s
        r = self.dropdown["rect"]
        pygame.draw.rect(s, (20, 22, 28), r)
        pygame.draw.rect(s, C_BTN_HL, r, 1)
        items = self.dropdown["items"]
        sc0 = self.dropdown["scroll"]
        for row, (lbl, _) in enumerate(items[sc0:sc0 + DD_MAX_VIS]):
            ir = pygame.Rect(r.x + 2, r.y + 2 + row * DD_ITEM_H, r.w - 4, DD_ITEM_H)
            if ir.collidepoint(*self.mouse_pos):
                pygame.draw.rect(s, (54, 70, 100), ir)
            s.blit(self.font_sm.render(lbl, True, C_TEXT), (ir.x + 8, ir.y + 4))
    ...
    def apply_param(self, param: str, new_val: float) -> None:
        ...
        if param == "duration":
            ...
            new_val = min(14.0, max(-14.0, new_val))
    ...
    def draw(self, surf: pygame.Surface) -> None:
        ...
        mp.draw_rulers(surf, self.w2s, self.s2w,
                       pygame.Rect(0, self.TOPBAR_H, self.W, self.CANVAS_H),
                       font=self.font_sm)
    ...
    def _draw_map(self) -> None:
        s = self._s
        s.fill(C_GRASS)
        mp.draw_map(s, self.scenario.map, self.w2s,
    ...
    def handle_event(self, event, local_pos: Optional[Tuple[int, int]] = None) -> None:
        ...
        elif event.type == pygame.MOUSEWHEEL and self.dropdown is not None:
            mx_sc = len(self.dropdown["items"]) - DD_MAX_VIS
            if mx_sc > 0:
                self.dropdown["scroll"] = max(
                    0, min(mx_sc, self.dropdown["scroll"] - event.y))
        elif event.type == pygame.KEYDOWN:
            self._handle_key(event)

    def _handle_key(self, event) -> None:
        if self.dropdown is not None and event.key == pygame.K_ESCAPE:
            self.dropdown = None
            return
    ...
    def _handle_click(self, mx, my) -> None:
        ...
        if not self.playing and self.selected is not None and self.cur_maneuver():
            ...
            if hb["type"].collidepoint(mx, my):
                self.open_dropdown(hb["type"], [(t, t) for t in MANEUVER_TYPES],
                                   self.do_set_maneuver_type)
                return
            m = self.cur_maneuver()
